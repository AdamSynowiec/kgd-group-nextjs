...
export default function Gallery({ fields }: { fields: GalleryFields }) {
  ...
  return (
    <div id="Wizualizacje" className="bg-[#1C1D21] text-white overflow-hidden">
      <Container>
        <h2 className="text-white font-ebgaramond-regular text-[40px] md:text-[64px] py-[50px] md:py-[100px]">{header}</h2>
      </Container>

      <div className="mx-auto px-4 pb-20">
        <div className="relative">
          <div
            className={`pointer-events-none absolute left-0 top-0 z-20 h-full w-24 bg-gradient-to-r from-[#1C1D21] via-[#1C1D21]/80 to-transparent transition-opacity duration-300 ${canScrollPrev ? "opacity-100" : "opacity-0"}`}
          />
          <div
            className={`pointer-events-none absolute right-0 top-0 z-20 h-full w-24 bg-gradient-to-l from-[#1C1D21] via-[#1C1D21]/80 to-transparent transition-opacity duration-300 ${canScrollNext ? "opacity-100" : "opacity-0"}`}
          />
          ...
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex">
              {images.map((img, index) => (
                <div key={img.path} className="min-w-[80%] md:min-w-[60%] lg:min-w-[58%] pl-3 pr-3">
                  <div onClick={() => handleImageClick(index)} className="group relative aspect-[16/10] overflow-hidden cursor-pointer bg-black/20">
