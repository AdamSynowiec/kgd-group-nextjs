...
type GalleryFields = {
  ...
  images?: EditableValue<GalleryImage[]> | GalleryImage[];
};

export default function Gallery({ fields }: { fields: GalleryFields }) {
  const header = unwrap(fields.header);
  const images = unwrap(fields.images) ?? [];
  ...
  return (
    <div id="wizualizacje" className="bg-[#1D1D1D] text-white overflow-hidden">
      <Container>
        <h2 className="text-white font-ranade-variable text-[40px] md:text-[64px] py-[50px] md:py-[100px]">{header}</h2>
      </Container>
      ...
      <div className="mx-auto px-4 pb-20">
        <div className="relative">
          ...
          <button
            ...
            className={`absolute left-3 top-1/2 -translate-y-1/2 z-30 h-12 w-12 flex items-center justify-center rounded-full backdrop-blur-xl bg-white/10 border border-white/10 transition hover:bg-white/20 hover:scale-105 ${canScrollPrev ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          >
          ...
          <button
            ...
            className={`absolute right-3 top-1/2 -translate-y-1/2 z-30 h-12 w-12 flex items-center justify-center rounded-full backdrop-blur-xl bg-white/10 border border-white/10 transition hover:bg-white/20 hover:scale-105 ${canScrollNext ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {images.map((img, index) => (
                <div key={img.path} className="min-w-[88%] md:min-w-[70%] lg:min-w-[58%] xl:min-w-[52%] pl-3 pr-3">
                  <div
                    ...
                    className="group relative aspect-[16/10] overflow-hidden cursor-pointer rounded-[28px] bg-black/20 border border-white/5 transition duration-500 hover:-translate-y-1 hover:border-white/10"
                  >
                    <img
                      loading="lazy"
                      decoding="async"
        ...
        {selectedImage && (
          ...
          >
            <button
              ...
              aria-label="Poprzednie zdjęcie"
              className="absolute left-4 md:left-8 z-50 h-14 w-14 flex items-center justify-center rounded-full bg-white/10 border border-white/10"
            >
            ...
            <button
              ...
              aria-label="Następne zdjęcie"
              className="absolute right-4 md:right-8 z-50 h-14 w-14 flex items-center justify-center rounded-full bg-white/10 border border-white/10"
            >
              <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M9 5l7 7-7 7" />
              </svg>
