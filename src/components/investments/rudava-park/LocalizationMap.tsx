...
const ICON_BASE = "/investments/rudava-park";
...
function buildIcons() {
  const icons: Record<string, L.Icon> = {
    inwestycja: L.icon({
      iconUrl: `${ICON_BASE}/rudava-park-map-pin.svg`,
      iconSize: [120, 120],
      iconAnchor: [60, 120],
    }),
  };
  ...
  for (const [type, file] of Object.entries(ICON_FILES)) {
    ...
    icons[type] = L.icon({
      iconUrl: `${ICON_BASE}/poi/${file}`,
      iconSize: size,
      iconAnchor: [size[0] / 2, size[1]],
    });
  }

  icons.willaDecjusza = icons.inwestycja;

  return icons;
}

export default function LocalizationMap({ points, center }: { points: MapPoint[]; center: [number, number] }) {
  // Bezpieczne jako lazy initializer: ten komponent renderuje się wyłącznie
  // po stronie klienta (next/dynamic z ssr:false w Localization.tsx), więc
  // `window`/Leaflet są tu zawsze dostępne, bez potrzeby efektu.
